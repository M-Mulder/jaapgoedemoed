...
export async function generateMetadata({
  ...
  const exhibitionsContent = getLocaleData(locale).exhibitions;

  return generateLocalizedMetadata({
    locale,
    path: '/exhibitions',
    title: `${exhibitionsContent.title} | Jaap Goedemoed`,
    description: exhibitionsContent.description,
  });
}
    ...
    <div className="container-narrow py-12 md:py-20 bg-[#0f0f0f]">
      ...
      <section className="relative h-[40vh] mb-16 overflow-hidden rounded-md">
        ...
        <div className="absolute bottom-0 left-0 p-8 w-full">
          <h1 className="page-title mb-2">{exhibitionsContent.title}</h1>
          <p className="text-white/80 max-w-xl">
            {exhibitionsContent.description}
          </p>
        </div>
      </section>
      ...
      <section className="mb-20">
        <h2 className="text-3xl font-medium text-[#f0f0f0] mb-8" style={{fontFamily: "var(--font-playfair), Georgia, serif"}}>
          {exhibitionsContent.pastExhibitions}
        </h2>
              ...
              <div className="flex flex-col md:flex-row md:justify-between md:items-start gap-6">
                <div className="md:w-1/3">
                  ...
                  <span className="inline-block mt-2 px-3 py-1 text-sm bg-[#252525] text-[#a3a3a3] rounded">
                    {exhibition.type === 'solo' ? exhibitionsContent.soloExhibition : exhibitionsContent.groupExhibition}
                  </span>
                </div>
      ...
      <section className="mt-20">
        <h2 className="text-3xl font-medium text-[#f0f0f0] mb-8 text-center" style={{fontFamily: "var(--font-playfair), Georgia, serif"}}>
          {exhibitionsContent.atelierImpressions}
        </h2>
